...
#[test]
fn can_be_created() {
    let id1 = FunctionId::new("/", "=001/001").unwrap();
    assert_eq!(format!("{}", id1), "=001/001");

    let id2 = FunctionId::new(".", "=1000.0002.0053").unwrap();
    assert_eq!(format!("{}", id2), "=1000.0002.0053");
}

#[test]
fn can_be_created_with_builder() {
    let mut builder = FunctionId::builder();
...
fn empty_cannot_be_created() {
    FunctionId::new("/", "").unwrap_or_else(|error| panic!("{}", error));
}

#[test]
#[should_panic(expected = "code group deviates in length")]
fn wrong_group_length_cannot_be_created() {
    FunctionId::new("/", "=0001/0200/012").unwrap_or_else(|error| panic!("{}", error));
}
...
fn wrong_prefix_cannot_be_created() {
    FunctionId::new("/", "+001.001").unwrap_or_else(|error| panic!("{}", error));
}

#[test]
fn equals_should_compare_equal() {
    let id1 = FunctionId::new(ID_SEGMENT_DELIMITER_DEFAULT, "=120.010.001").unwrap();
    let id2 = FunctionId::new(ID_SEGMENT_DELIMITER_DEFAULT, "=120.010.001").unwrap();
    assert!(id1 == id2);
}
...
fn unequals_should_compare_not_equal() {
    let id1 = FunctionId::new(ID_SEGMENT_DELIMITER_DEFAULT, "=120.010.001").unwrap();
    let id2 = FunctionId::new(ID_SEGMENT_DELIMITER_DEFAULT, "=120.010.002").unwrap();
    assert!(id1 != id2);
}
